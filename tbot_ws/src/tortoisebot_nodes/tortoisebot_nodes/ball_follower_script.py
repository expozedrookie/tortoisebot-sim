...
class BallFollowNode(Node):
    ...
    def __init__(self):
        ...
        # Add a timer for the control loop
    ...
    def moveRobot(self,angularTarget=0.0,LinearTarget=0.0,stopRobot=False):
        ...
        if distDiff!=0.0 :
            linearVel=gainLinearVel*((2/(1+exp(-4*distDiff)))-1)*max(0.0, 0.8- (abs(angularTarget)))
            # linearVel=gainLinearVel*(distDiff)*(1-abs(angularTarget/pi))            
        # self.get_logger().warn('[DEBUG][BALL_FOLLOW_NODE] l a : %s %s'%(linearVel,angularVel))
        ...
        if linearVel in [2.0,-2.0] or angularVel in [2.0,-2.0]:
            if linearVel in [2.0,-2]:
                limiterName='LINEAR'
    ...
    def cmdvel_callback(self):
        ...
        if self.noObjectFlag==False:
            ...
            self.get_logger().warn('[DEBUG][BALL_FOLLOW_NODE] PUBLISHING %s %s'%(resultMove))
